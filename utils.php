<?php
	...
	function getGroupImage($conn, $groupname) {
		if ($result = mysqli_query($conn, "SELECT `image` FROM `groups` WHERE `name` LIKE '".$groupname."'")) {
			if (mysqli_num_rows($result) == 0) {
				return "";
			} else {
				$row = mysqli_fetch_array($result);
	...
	function doRegister($conn, $username, $password, $fname, $lname, $email, &$register_notify) {
		if ($result = mysqli_query($conn, "SELECT `id` FROM `users` WHERE `username` LIKE '".$username."' OR `email` LIKE '".$email."'")) {
			if (mysqli_num_rows($result) == 0) {
				$hashed = hash("sha256", "SUPERSECRETSALT".$password);
				if (mysqli_query($conn, "INSERT INTO `users` (`id`, `username`, `password`, `first name`, `last name`, `email`, `image`) VALUES (NULL, '$username', '$hashed', '$fname', '$lname', '$email', '')"))
