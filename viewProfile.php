<?php
	...
	if (!isset($_GET["user"])) {
		...
		createGroup($conn, $_POST["groupName"], $_POST["groupDesc"], $_POST["groupImage"], $createGroup_notify);
...
<?php
			...
			if (isset($_SESSION["username"]) && strcmp($_SESSION["username"], $_GET["user"]) == 0) {
				...
				echo '</div>';
				echo '<div class="form-group">';
				echo '<label for="img_in">Group Image URL:</label>';
				echo '<input type="text" name="groupImage" class="form-control" id="img_in" placeholder="Group Image">';
				echo '</div>';
